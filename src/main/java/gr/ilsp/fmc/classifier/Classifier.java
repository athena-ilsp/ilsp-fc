...
//import gr.ilsp.fmc.main.Bitexts;
...
public class Classifier implements Serializable{
	...
	/*public double rankLink(String text1) {
		...
		try {
			...
			//vpapa
			String[] langs = _targetLanguage.split(";");
			//score+=Bitexts.heuristitlinkscore(text,langs);
			...
			for (String lang:langs){
				//if (text.equals(lang)){
				if (containLangKeys(text,m)){
					match = true;
					System.out.println("BINGO");
					langidentified_link=lang;
					break;
				}
			...
			if (match){
				if (langidentified_link.equals(langIdentified)){
					System.out.println(langidentified_link +" "+langIdentified);
					score += _thres;
				}
				else{
					if (match1){
