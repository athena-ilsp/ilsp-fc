...
public class ILSPFCUrlNormalizer extends SimpleUrlNormalizer {
    private static final Logger LOGGER = LoggerFactory.getLogger(ILSPFCUrlNormalizer.class);
    ...
    private static final Pattern AGGRESSIVE_IGNORED_QUERY_PARAMETERS_PATTERN = 
    		Pattern.compile("(\\?|&)(?:(?i)user|usr|user_id|userid|memberid)=.*?(&|#|$)");
    
    private boolean _treatRefAsQuery;
    private boolean _isAggressive;
    
    public ILSPFCUrlNormalizer() {
    	this(false, false);
    }
    
    public ILSPFCUrlNormalizer(boolean treatRefAsQuery) {
        this(treatRefAsQuery, false);
    }
    
    public ILSPFCUrlNormalizer(boolean treatRefAsQuery, boolean isAggressive) {
