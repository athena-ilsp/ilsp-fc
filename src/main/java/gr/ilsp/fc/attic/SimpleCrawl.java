...
public class SimpleCrawl {
	...
	public static void main(String[] args) {
		...
		double relthres = Crawl.config.getDouble("classifier.relative_relevance_threshold.value");
			...
			long targetEndTime = hasEndTime ? 
					...
					for (int curLoop = startLoop + 1; curLoop <= endLoop; curLoop++) {
						...
						Flow flow = SimpleCrawlWorkflow.createFlow(inputPath, curLoopDir, userAgent, defaultPolicy, urlFilter, 
								options.getThreads(), options.isDebug(), options.getDbHost(), options.getDbName(), options.getLanguage().split(lang_separator),options.getLangKeys(),
